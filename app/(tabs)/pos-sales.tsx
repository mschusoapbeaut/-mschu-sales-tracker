...
// Define the Sale type locally to match the schema
type Sale = {
  id: number;
  userId: number;
  productName: string;
...
export default function POSSalesScreen() {
  ...
  const { data: salesList, isLoading: listLoading, refetch: refetchList } = trpc.sales.posList.useQuery(
    { period },
    { enabled: isAuthenticated }
  );

  const { data: summary, isLoading: summaryLoading, refetch: refetchSummary } = trpc.sales.posSummary.useQuery(
    { period },
    { enabled: isAuthenticated }
  );
  ...
  const totalNetSales = salesList?.reduce((sum: number, sale: Sale) => sum + parseFloat(sale.totalAmount || "0"), 0) || 0;
    ...
    <ScreenContainer>
      ...
      >
        ...
        <View className="px-5 pt-4 pb-2">
          <Text className="text-2xl font-bold text-foreground">POS Sales</Text>
          <Text className="text-sm text-muted">Your point-of-sale transactions</Text>
        </View>
        ...
        {/* Table Header - includes Payment Gateway column */}
        <View className="px-5 pt-4 pb-2">
          <View className="bg-surface rounded-t-xl border border-border p-3">
            <View className="flex-row">
              <Text className="w-20 text-xs font-semibold text-muted">Date</Text>
              <Text className="w-16 text-xs font-semibold text-muted text-center">Order</Text>
              <Text className="flex-1 text-xs font-semibold text-muted text-center">Channel</Text>
              <Text className="w-20 text-xs font-semibold text-muted text-center">Payment</Text>
              <Text className="w-20 text-xs font-semibold text-muted text-right">Net Sales</Text>
            </View>
          </View>
        </View>
        ...
        <View className="px-5">
          {isLoading ? (
            ...
            <View className="bg-surface border-x border-b border-border rounded-b-xl overflow-hidden">
              {salesList.map((sale: Sale, index: number) => (
                <View
                  key={sale.id}
                ...
                >
                  <View className="flex-row items-center">
                    {/* Order Date */}
                    <View className="w-20">
                      <Text className="text-xs text-foreground">{formatDate(sale.saleDate)}</Text>
                    </View>
            ...
            <View className="bg-surface border-x border-b border-border rounded-b-xl p-8 items-center">
              <MaterialIcons name="point-of-sale" size={48} color={colors.muted} />
              <Text className="text-muted mt-2 text-center">No POS transactions found</Text>
            </View>
          )}
        </View>
      </ScrollView>
